...
Option A – via moon command: `moon api-ticketing:install-mockery`
...
### Instrumentation
Tracing is enabled exclusively in the production environment. Set `APP_ENVIRONMENT` to `production` to activate tracing. Alternatively, you may customize the tracing rules in `apps/api-ticketing/config/tracer.go`.

For instructions on customizing span tracing, please refer to the example located at:
- `apps/api-ticketing/internal/rest/user.go`
